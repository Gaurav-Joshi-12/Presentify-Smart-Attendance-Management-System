...
import { c as cn, N as Navbar } from "./Navbar-CPIRaQSZ.js";
import { C as Card } from "./api-Rs_hdI2y.js";
import { S as Select } from "./Input-unOD1WgV.js";
...
function AttendanceReport({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "glass rounded-xl overflow-hidden", children: [
    /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-12 gap-3 px-4 py-3 text-[11px] uppercase tracking-wider text-muted-foreground border-b border-white/5", children: [
      /* @__PURE__ */ jsx("div", { className: "col-span-1", children: "Sr No" }),
    ...
    records.map((r, index) => {
      ...
      return /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-12 gap-3 px-4 py-3 row-hover border-b border-white/5 items-center", children: [
        /* @__PURE__ */ jsx("div", { className: "col-span-1 text-[11px] text-muted-foreground", children: index + 1 }),
        /* @__PURE__ */ jsx("div", { className: "col-span-2 font-mono text-xs", children: s?.rollNo || "—" }),
        /* @__PURE__ */ jsx("div", { className: "col-span-4 text-sm", children: s ? `${s.firstName} ${s.lastName}` : "Unknown" }),
        ...
        /* @__PURE__ */ jsx("div", { className: "col-span-3 text-sm text-muted-foreground", children: r.remarks || "—" })
...
function Reports() {
  ...
  useEffect(() => {
    ...
    profService.listLecturesBySubject(Number(subjectId)).then((lecs) => {
      const sorted = [...lecs].sort((a, b) => (/* @__PURE__ */ new Date(`${a.lectureDate}T${a.startTime}`)).getTime() - (/* @__PURE__ */ new Date(`${b.lectureDate}T${b.startTime}`)).getTime());
      setLectures(sorted);
  ...
  useEffect(() => {
    ...
    Promise.all([profService.attendanceByLecture(Number(lectureId)).catch(() => []), profService.listClassStudents({
      ...
      setRecords(recs);
      setStudents(st);
      Promise.all(st.map((s) => profService.studentPercentage(s.studentId, Number(subjectId)).then((p) => [s.studentId, p]).catch(() => [s.studentId, 0]))).then((pairs) => setSubjectPercentages(Object.fromEntries(pairs)));
      Promise.all(st.map((s) => profService.studentPercentage(s.studentId).then((p) => [s.studentId, p]).catch(() => [s.studentId, 0]))).then((pairs) => setOverallPercentages(Object.fromEntries(pairs)));
    }).finally(() => setLoading(false));
  ...
  const flagged = useMemo(() => students.filter((s) => (subjectPercentages[s.studentId] ?? 0) < 75), [students, subjectPercentages]);
    ...
    /* @__PURE__ */ jsxs("main", { className: "mx-auto w-[min(1280px,calc(100%-2rem))] mt-8 space-y-5", children: [
      ...
      /* @__PURE__ */ jsx(Card, { children: /* @__PURE__ */ jsxs("div", { className: "grid sm:grid-cols-3 gap-4", children: [
        ...
        /* @__PURE__ */ jsxs(Select, { label: "Lecture", value: lectureId, onChange: (e) => setLectureId(e.target.value), children: [
          ...
          lectures.map((l, i) => /* @__PURE__ */ jsxs("option", { value: l.lectureId, children: [
            "Lec ",
            i + 1,
            " : ",
            l.topic
      ...
      /* @__PURE__ */ jsx(Card, { title: "Lecture roster", subtitle: lectureId ? (() => {
        const idx = lectures.findIndex((l2) => String(l2.lectureId) === lectureId);
      ...
      })() : "Select a lecture", children: loading ? /* @__PURE__ */ jsxs("div", { className: "py-10 flex items-center justify-center gap-2 text-muted-foreground", children: [
        /* @__PURE__ */ jsx(Loader2, { className: "h-4 w-4 animate-spin" }),
        "Loading…"
      ] }) : /* @__PURE__ */ jsx(AttendanceReport, { records, students }) }),
      /* @__PURE__ */ jsx(Card, { title: "Subject Attendance", subtitle: "Students below 75% in this subject are flagged", actions: flagged.length > 0 && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2 text-xs px-3 py-1.5 rounded-full badge-danger-glow", children: [
        /* @__PURE__ */ jsx(AlertTriangle, { className: "h-3.5 w-3.5" }),
        " ",
        flagged.length,
        " at risk"
      ] }), children: students.length === 0 ? /* @__PURE__ */ jsx("div", { className: "py-10 text-center text-sm text-muted-foreground", children: "No students." }) : /* @__PURE__ */ jsx("div", { className: "grid sm:grid-cols-2 lg:grid-cols-3 gap-3", children: students.map((s) => /* @__PURE__ */ jsx(PctTile, { student: s, pct: subjectPercentages[s.studentId] ?? 0 }, s.studentId)) }) }),
      /* @__PURE__ */ jsx(Card, { title: "Overall Attendance", subtitle: "Cumulative attendance across all subjects", children: students.length === 0 ? /* @__PURE__ */ jsx("div", { className: "py-10 text-center text-sm text-muted-foreground", children: "No students." }) : /* @__PURE__ */ jsx("div", { className: "grid sm:grid-cols-2 lg:grid-cols-3 gap-3", children: students.map((s) => /* @__PURE__ */ jsx(PctTile, { student: s, pct: overallPercentages[s.studentId] ?? 0 }, s.studentId)) }) })
    ] })
  ] });
}
function PctTile({
